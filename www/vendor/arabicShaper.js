/* ==========================================================================
   Vendored from arabic-persian-reshaper (ArabicShaper.js), MIT License,
   Copyright (c) 2018 Shen Yiming — https://github.com/soimy/arabic-persian-reshaper
   Adapted for direct <script> use (was a CommonJS module): wrapped in an
   IIFE and exposed as window.ArabicShaper instead of module.exports.

   Converts logical-order Arabic text into its correct contextual
   presentation-form glyphs (isolated/initial/medial/final) — jsPDF has no
   Arabic text shaping of its own, so without this every Arabic character
   would render in its isolated form regardless of position, which reads
   as disconnected, wrong-looking letters rather than joined script. Used
   by buildReceiptPDF() in app.js together with bidi.min.js (visual
   reordering) before handing text to jsPDF.
   ========================================================================== */
(function () {
/**
 *
...
function convertArabicBack(apfb) {
  ...
  window.ArabicShaper = { convertArabic: convertArabic, convertArabicBack: convertArabicBack };
})();
